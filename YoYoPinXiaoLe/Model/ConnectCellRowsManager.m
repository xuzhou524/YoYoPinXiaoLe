...
@implementation ConnectCellRowsManager
...
+(NSArray*)detectConnectedRowsVerticallyWithGraph:(Graph*)graph withVertixe:(GraphCell*)VertixGCell{
    ...
    for(int i = 0;i<graph.size.height;i++){
        BOOL breakLoop = [self detectCellsInaRowWithConnectedCellsArray:result withCurrentGCell:[graph getGraphCellWithX:StartX withY:StartY+i]];
        if(breakLoop){
            break;
        }
    }
...
+(NSArray*)detectConnectedRowsHorizontallyWithGraph:(Graph*)graph withVertixe:(GraphCell*)VertixGCell{
    ...
    for(int i = 0;i<graph.size.width;i++){
        BOOL breakLoop = [self detectCellsInaRowWithConnectedCellsArray:result withCurrentGCell:[graph getGraphCellWithX:StartX+i withY:StartY]];
        if(breakLoop){
            break;
        }
    }
...
+(NSArray*)detectConnectedRowsDiagonallyToTheRightWithGraph:(Graph*)graph withVertixe:(GraphCell*)VertixGCell{
    ...
    for(int i = 0 ;StartX+i<=EndX||StartY-i>=EndY;i++){
        BOOL breakLoop = [self detectCellsInaRowWithConnectedCellsArray:result withCurrentGCell:[graph getGraphCellWithX:StartX+i withY:StartY-i]];
        if(breakLoop){
            break;
        }
    }
...
+(BOOL)detectCellsInaRowWithConnectedCellsArray:(NSMutableArray*)ConnectedCells withCurrentGCell:(GraphCell*)CurrentCell{
